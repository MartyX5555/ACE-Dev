...
function EFFECT:Init( data ) 
	...
	local WaterTr = { }
		...
		local startposition = self.Origin + Vector(0,0,1000 * self.Radius)
	...
	if self.Radius >= 10 then
		if not self.HitWater and not self.UnderWater then
			sound.Play( "acf_mac_sounds/dist_close_"..math.random(1,3)..".wav", self.Origin , 107, math.Clamp(self.Radius*10,75,165), math.Clamp(300 - self.Radius*12,15,255))
		elseif self.HitWater and not self.UnderWater then
			...
			sound.Play( "^weapons/underwater_explode3.wav", self.Origin, math.Clamp(self.Radius*10,50,100), math.Clamp(300 - self.Radius*12,100,255))
		end
	else
		if not self.HitWater then
			sound.Play( "acf_other/explosions/cookOff"..math.random(1,4)..".wav", self.Origin , 107, math.Clamp(self.Radius*10,75,165), math.Clamp(300 - self.Radius*25,15,255))
		end
	end

	sound.Play( "acf_mac_sounds/dist_medium_"..math.random(1,3)..".wav", self.Origin , 152.8, math.Clamp(self.Radius*10,75,165), math.Clamp(300 - self.Radius*25,15,255))
	sound.Play( "acf_mac_sounds/dist_far_"..math.random(1,3)..".wav", self.Origin , 180, math.Clamp(self.Radius*10,75,165), math.Clamp(300 - self.Radius*25,15,255))
