...
function ACF_RoundBaseGunpowder( PlayerData, Data, ServerData, GUIData )
	...
	local GunData 		= Guns[PlayerData.Id]

	local RoundData, MaxLenght, PropWeight, Caliber

	if GunData then

		Caliber 	= GunData.caliber
		RoundData 	= GunData.round

		MaxLenght 	= RoundData.maxlength
		PropWeight 	= RoundData.propweight
	else

		if PlayerData.Custom then

			Caliber 	= PlayerData.Custom.caliber
			MaxLenght 	= PlayerData.Custom.maxlength
			PropWeight 	= PlayerData.Custom.propweight
		else
			error("trying to use the functio with an invalid gun!!!")
		end
	end

	GUIData.MaxTotalLength 		= MaxLenght * ( Data.LengthAdj or 1)
		
	Data.Caliber 				= Caliber
	Data.FrAera 				= 3.1416 * ( Data.Caliber/2 ) ^ 2
	Data.Tracer 				= PlayerData.Data10 > 0 and math.min( Data.Caliber/5, 3 ) or 0
	...
	local PropMax 	= ( PropWeight * 1000/ACF.PDensity) / Data["FrAera"]	--Current casing absolute max propellant capacity
