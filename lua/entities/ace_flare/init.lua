	...
	local function FindClutter( self )
		...
		do
			...
			for k, flare in ipairs(nflares) do
				if not IsValid(flare) then continue end                    --skips invalid ents
				if flare:GetClass() ~= "ace_flare" then continue end       --skips any other class
				if flare:EntIndex() == self:EntIndex() then continue end   --skips the own flare
